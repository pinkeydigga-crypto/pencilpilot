...
export default function ChallengesPage() {
  ...
  // Fetch real authenticated user and leaderboard data from Supabase with auto-profile sync
  useEffect(() => {
    async function fetchData() {
      if (!supabase) return;
      
      // 1. Get real logged-in user from Supabase Auth
      ...
      // 2. Check if user profile exists; if not, automatically create one so they appear on leaderboard
      ...
      if (fetchErr) {
        console.error("Profile fetch error:", fetchErr.message, fetchErr.details);
      }
      ...
      if (!userData) {
        ...
        if (createErr) {
          console.error("Profile creation error code:", createErr.code);
          console.error("Profile creation error message:", createErr.message);
          console.error("Profile creation error details:", createErr.details);
        }
      ...
      // 3. Fetch ALL registered users for leaderboard
      ...
      if (lbError) {
        console.error("Error fetching leaderboard:", lbError.message, lbError.details);
      }
  ...
  // Handle completing challenge (+100 XP & Streak calculation saved permanently)
  const handleFinishChallenge = async () => {
    if (!userId) return;

    // Agar pehle hi completed hai, toh dobara XP add mat karo bas hub pe le jao
    if (isCompleted) {
      setActiveView('hub');
      return;
    }
    ...
    if (supabase) {
      try {
        ...
        console.error("Unexpected error saving XP to Supabase:", err);
      }
    }
  };
    ...
    <div className="min-h-screen font-sans bg-gradient-to-br from-blue-600 to-blue-800 text-slate-900 p-6 md:p-10 flex flex-col items-center">
      <div className="max-w-4xl w-full space-y-8">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-md flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Master drawing skills and compete on the community leaderboard.</p>
            </div>
          </div>
          <button 
        ...
        {activeView === 'hub' ? (
          <>
            {/* Hero Stats Card */}
            <div className="bg-blue-900 text-white p-8 rounded-3xl shadow-xl border border-blue-800 space-y-6 flex flex-col md:flex-row justify-between items-center">
              <div className="space-y-3 max-w-lg text-center md:text-left">
                ...
                <h1 className="text-3xl md:text-4xl font-black tracking-tight text-white">Improve Your Drawing Skills</h1>
                ...
                <div className="space-y-6">
                  ...
                  {leaderboard.length >= 1 && (
                    <div className="flex justify-center items-end gap-3 pt-6 pb-4">
                      ...
                      {/* 1st Place (Center) */}
                      {leaderboard[0] && (
                        <div className="flex flex-col items-center space-y-2 w-32 -mt-6">
                          <div className="relative">
